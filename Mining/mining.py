...
def cloneRepos(repo_list, dev_threshold=10, python_threshold=0.50, commit_threshold = 5): 
    counter = 0     
    str_ = ''
    all_list = []
    for repo_batch in repo_list:
        for repo_ in repo_batch:
            counter += 1 
            print('Cloning ', repo_ )
            dirName = 'NON_ML_REPOS/' + repo_.split('/')[-2] + '@' + repo_.split('/')[-1] ## '/' at the end messes up the index 
...
if __name__=='__main__':
    repos_df = pd.read_csv('GITHUB_REPOS.csv', sep='delimiter')
